...
export async function addItemToCart({ cartId, variantId, quantity }) {
  ...
  try {
    ...
    console.log('Response from addItemToCart:', responseJson);

    if (
      !responseJson.data ||
      !responseJson.data.cartLinesAdd ||
      !responseJson.data.cartLinesAdd.cart ||
      !responseJson.data.cartLinesAdd.cart.lines ||
      !responseJson.data.cartLinesAdd.cart.lines.edges
    ) {
...
export async function removeItemFromCart(cartId, lineId) {
  ...
  try {
    const responseJson = await shopifyFetch(graphqlQuery, variables);
    console.log('Response from removeItemFromCart:', responseJson);
    
    if (
      !responseJson.data ||
      !responseJson.data.cartLinesRemove ||
      !responseJson.data.cartLinesRemove.cart ||
      !responseJson.data.cartLinesRemove.cart.lines ||
      !responseJson.data.cartLinesRemove.cart.lines.edges
    ) {
      throw new Error('Invalid response structure');
    }
...
export async function createCheckout(items) {
  ...
  try {
    const responseJson = await shopifyFetch(graphqlQuery, variables);
    console.log('Response from createCheckout:', responseJson);
...
export async function createCart() {
  ...
  try {
    ...
    console.log('Response from createCart:', responseJson);
...
export async function getProductsInCollection(collectionHandle) {
  ...
  try {
    const responseJson = await shopifyFetch(graphqlQuery, variables);
    console.log('Response from getProductsInCollection:', responseJson);
...
export async function getProduct(handle) {
  ...
  try {
    const responseJson = await shopifyFetch(graphqlQuery, variables);
    if (responseJson.data && responseJson.data.productByHandle) {
      ...
      return {
        ...
        imageSrc: product.images.edges[0]?.node.originalSrc || "/fallback-image.jpg",
        imageAlt: product.images.edges[0]?.node.altText || "Product Image",
        price: variant.priceV2?.amount || "0",
        currencyCode: variant.priceV2?.currencyCode || "USD",
...
export async function updateCheckout(cartId, lineItems) {
  ...
  try {
    const responseJson = await shopifyFetch(graphqlQuery, variables);
    console.log('Response from updateCheckout:', responseJson);

    if (
      !responseJson.data ||
      !responseJson.data.checkoutLineItemsReplace ||
      !responseJson.data.checkoutLineItemsReplace.checkout ||
      !responseJson.data.checkoutLineItemsReplace.checkout.lineItems ||
      !responseJson.data.checkoutLineItemsReplace.checkout.lineItems.edges
    ) {
      throw new Error('Invalid response structure');
    }

    return responseJson.data.checkoutLineItemsReplace.checkout;
  } catch (error) {
