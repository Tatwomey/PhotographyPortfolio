...
export async function createCheckout(lineItems) {
  ...
  try {
    ...
    console.log('Shopify response for checkoutCreate:', response); // Detailed logging

    if (response && response.data && response.data.checkoutCreate && response.data.checkoutCreate.checkout) {
      const checkout = response.data.checkoutCreate.checkout;
      console.log('Checkout session:', checkout); // Log checkout session details
      if (checkout && 'webUrl' in checkout && checkout.webUrl) {
        return {
            cartId: checkout.id,
            checkoutUrl: checkout.webUrl
        };
    } else {
        console.error('Checkout URL is undefined or not found in the response:', checkout);
        return { cartId: null, checkoutUrl: null };
    }
