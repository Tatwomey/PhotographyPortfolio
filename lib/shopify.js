...
export async function shopifyFetch(query, variables = {}) {
  ...
  console.log('SHOPIFY_API_ENDPOINT:', endpoint);
  console.log('SHOPIFY_API_TOKEN:', token);
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Error fetching from Shopify: ${res.statusText}`);
    }
    ...
    if (!responseText) {
      throw new Error('Error fetching from Shopify: Empty response');
    }
